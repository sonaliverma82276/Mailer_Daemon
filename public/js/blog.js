...
for (var i = 0; i < buttons_list.length; i++) {
  //assigned id to read more button
  buttons_list[i].setAttribute("id", i);
  //console.log("the id set for button "+i+" is "+ i);
}
for (var i = 0; i < buttons_like.length; i++) {
  //assigned id to like button
...
function closeme() {
  //closing function for modals
  document.getElementById("container1").classList.remove("show");
  document.getElementById("container1").classList.add("hide");
}

//LIKES BUTTON JS
function like(id) {
  //function for the like buttons
